	...
	\begin{cjsection}{}
		...
		not intersect the x-axis. How about if the graph is to intersect the x-axis at exactly one point?}
			\begin{cjsolution}
				...
				\cjcontinue{\(k^2 - 16 < 0\)}
				\cjsubwhy{\((k - 4)(k + 4) < 0\)}{Factor using difference of two squares.}
				\cjsign{
					\tkzTabInit[lgt=3, espcl=2, deltacl=0]
						{/.8, \(k - 4\) /.8, \(k + 4\) /.8, \((k - 4)(k + 4)\) /.8}
						{,\(-4\),\(4\),}
					\tkzTabLine{,-,t,-,t,+,}
					\tkzTabLine{,-,t,+,t,+,}
					\tkzTabLine{,+,t,-,t,+,}
				...
				\cjsubwhy{\((k - 4)(k + 4) = 0\)}{Factor using difference of two squares.}
				\cjcontinue{\(k \in \{-4, 4\}\)}
				\cjfastep{\(k \in (-4, 4)\)}{If the graph should have no x-intercept.}
				\cjfastep{\(k \in \{-4, 4\}\)}{If the graph should have exactly one x-intercept.}
			\end{cjsolution}
	\end{cjsection}
\end{document}
